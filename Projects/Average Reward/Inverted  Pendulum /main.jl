...
const theta_min=-35.0
const theta_max=35.0
const theta_dot_min= -65.0
const theta_dot_max=  65.0
...
function dynamics_rand(theta::Float64, theta_dot::Float64, u::Float64)
    if !is_safe(theta, theta_dot)
        return (theta, theta_dot)   # no movement if outside the safe region
    else
        d  = rand(Normal(0, sigma))
        g=10
        m=2
        l=1
        theta_next = theta + theta_dot
        theta_dot_next = theta_dot + (g/l*sin(theta) + (1/m*l^2)*u) + d
