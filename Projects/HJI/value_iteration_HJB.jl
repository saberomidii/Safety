...
for computing reachable sets, applied to a double integrator example.
Based on the paper by Akametalu et al. (2024)
"""

# System parameters
function double_integrator_dynamics(x, u)
    return [x[2], u]
end
...
# Value iteration using the Minimum Discounted Reward formulation
function value_iteration_mdr()
        ...
        for i in 1:nx1
            for j in 1:nx2
                ...
                for u_idx in 1:nu
                    ...
                    # Maximize over control
                    opt_val = max(opt_val, discounted_val)
                end
...
function plot_results(Z, l)
    ...
    p = plot(size=(800, 600), xlabel="x₁", ylabel="x₂", title="Safe Region for Double Integrator")
    ...
    savefig(p, "double_integrator_safe_region.png")
    return p
end

# Function to save the computed safe region as CSV
function save_safe_region_as_csv(Z, x1_grid, x2_grid, filename="safe_region.csv")
...
function save_analytical_safe_region_as_csv(Z, l, z_filename="Z_matrix.csv", l_filename="l_matrix.csv")
    ...
    coords_z_filename = "Z_with_coords.csv"
...
function main()
    ...
    # First run value iteration to get Z and l
    println("Computing value function for double integrator...")
    Z, l = value_iteration_mdr()
    ...
    computed_file_path = joinpath(pwd(), "computed_safe_region.csv")
