...
function solve_case()   
    ...
    # c2: flow constraints
    @constraint(model, c1[j in 1:nstates],
        g[j]>= sum(T[s,a,j]*g[s] for s in 1:nstates, a in 1:nactions)
    )


    # c2: flow constraints
    @constraint(model, c2[j in 1:nstates],
        g[j] + h[j]>= reward[j]+sum(T[s,a,j]*h[s] for s in 1:nstates, a in 1:nactions)
    )
    ...
    @objective(model, Min, sum(alpha_dist*g[s] for s in 1:nstates))
   ...
   for s in 1:nstates
    ...
    return objective_value(model), X, Y, g_sol, h_sol
end
...
function main_3D()
    ...
    if !isnothing(G)
        writedlm(joinpath(results_dir, "G.csv"), G, ',')
    end
    
    if !isnothing(H)
	    writedlm(joinpath(results_dir, "H.csv"), H, ',')
    end
