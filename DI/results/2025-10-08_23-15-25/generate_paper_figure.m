function generate_paper_figure()
    ...
    contour(ax, velocity_coords, position_coords, Z_lambda0_0.', [0.0 0.0], 'LineWidth', 3.5, 'Color', [0.8500 0.3250 0.0980], 'LineStyle', '-', 'DisplayName', 'MDR ($\lambda=0.0$)');
    contour(ax, velocity_coords, position_coords, Z_lambda0_01.', [epsilon, epsilon], 'LineWidth', 3, 'Color', [0 0.4470 0.7410], 'LineStyle', '--', 'DisplayName', 'MDR ($\lambda=0.01$)');
    contour(ax, velocity_coords, position_coords, Z_lambda0_1.', [epsilon, epsilon], 'LineWidth', 3, 'Color', [0.4660 0.6740 0.1880], 'LineStyle', ':', 'DisplayName', 'MDR ($\lambda=0.015$)');
    contour(ax, velocity_coords, position_coords, Z_lambda0_2.', [epsilon,epsilon], 'LineWidth', 3, 'Color', [0.4940 0.1840 0.5560], 'LineStyle', '-.', 'DisplayName', 'MDR ($\lambda=0.02$)');
...
function formatAxesAndLabels(ax)
    ...
    xlim(ax, [-3.5, 3.5]); % New X-axis (Velocity) limits
    ...
    new_left = 0.35;  % Move it slightly right (5% from left edge of figure)
    new_bottom = 0.35; % Move it up (30% from bottom edge of figure)
