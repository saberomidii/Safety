    ...
    for a in 1:nactions
        ...
        for i in 1:nsamples
            ...
            # while j>1
                ...
                idxs, dists = knn(tree, [xn, vn], 4)

                # println(dists)
                dists= dists/norm([x1[1]-x1[2],x2[1]-x2[2]])
                # println(dists)
                # if first(idxs) != is  || j>100
                    for (i,d) in zip(idxs, dists)
                        T[is, a, i] += d/(nsamples*sum(dists))
                                    #    println(d/(nsamples*sum(dists)))
                    end
                # j += 1
            # end
        end
...
function solve_primal_case()   
    ...
    set_optimizer_attribute(model, "FeasibilityTol", 1e-6)  # Feasibility tolerance
    set_optimizer_attribute(model, "OptimalityTol", 1e-6)   # Optimality tolerance
    set_optimizer_attribute(model, "MIPGap", 1e-6)          # Relative MIP gap tolerance (1%)
    set_optimizer_attribute(model, "BarConvTol", 1e-6)      # Barrier convergence tolerance
    ...
    set_optimizer_attribute(model, "BarIterLimit", 300)      # Limit barrier iterations
    set_optimizer_attribute(model, "IterationLimit",300)    # Limit simplex iterations
    ...
    set_optimizer_attribute(model, "TimeLimit", 200)         # 60 second time limit
    
    # Round solution to 2 decimal places
    set_optimizer_attribute(model, "PSDTol", 1e-6)          # Positive semi-definite tolerance
...
function main_primal()
    ...
    results_dir = joinpath(@__DIR__, "results_primal_interpolation", string(sigma))
    mkpath(results_dir)
